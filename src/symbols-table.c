...
static TPCType to_tpc(Node *n)
{
    ...
    TPCType type;
    if (strcmp(n->u.identifier, "int") == 0)
    { // int type
        type.u.ptype = TPCInt;
        type.kind = KindPrim;
    }
    else if (strcmp(n->u.identifier, "char") == 0)
    { // char type
...
static int add_declstruct(SymbolsTable *table, Node *node)
{
    ...
    for (n = FIRSTCHILD(node); n; n = n->nextSibling)
    { // add all fields of the struct to the table
...
static TPCType to_tpc_ftype(SymbolsTable *table, Node *n)
{
    ...
    else
    {
        ...
        if (strcmp(FIRSTCHILD(tmp)->u.identifier, "int") == 0)
        { // return int
            type.u.ftype.return_type.kind = KindPrim;
            type.u.ftype.return_type.u.ptype = TPCInt;
        }
        else if (strcmp(FIRSTCHILD(tmp)->u.identifier, "char") == 0)
        { // return char
            type.u.ftype.return_type.kind = KindPrim;
        ...
        { // return struct
            ...
            strcpy(type.u.ftype.return_type.u.struct_name, FIRSTCHILD(tmp)->u.identifier);
        }
    }
    ...
    for (args = FIRSTCHILD(tmp); args; args = args->nextSibling)
    {
        ...
        if (args->kind == Primitive)
        {
            ...
            type_func.u.ptype = !strcmp(args->u.identifier, "int") ? TPCInt : TPCChar;
        }
